...
Для нод под runr:
    ...
    cat > /etc/runr/bosun.service <<'EOF'
    [Service]
...
Для нод под systemd:
    ...
    cat > /etc/systemd/system/bosun.service <<'EOF'
    [Unit]
    ...
    cat > /etc/systemd/system/bosun.timer <<'EOF'
    [Unit]
...
Простейший вариант для тестовых стендов:
...
Подробности см. в [bundle-authoring.md](bundle-authoring.md), здесь
operator-shortcuts.
...
- **Tmpfs by design.** Reboot обнуляет `/tmp/bosun-defers/`.
- **Дедуп.** `restart > reload_or_restart > reload` в пределах одного
  `(action, target)`. Внутри прогона дубликаты не накапливаются.
- **Max attempts.** Дефолт 3 (`--defer-max-attempts`). При 4-й попытке
  переименование в `.manual_clear`.
