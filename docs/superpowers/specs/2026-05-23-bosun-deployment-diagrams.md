...
```
                              ...
                              └────┬─────┘      └────┬─────┘      └────┬─────┘
                                  ...
                                  └────────┬─────────┘
                                           │
                                           │ Pods polling каждые 500ms
                                           │ командов для своих hosts
                                           │
                                ╔══════════╧══════════════════════════════╗
...
```
                       ...
                       │   (host, pod_id=pod_2, ...)       │
                       │ → trigger command-dispatch loop   │
                       │   (читает первую порцию для       │
                       │    нового host'а без 500ms wait)  │
                       └───────────────┬───────────────────┘
                                       │
                                       ▼
                       ┌───────────────────────────────────┐
...
│ для каждого host'а — на           │
│ следующем 500ms polling-tick'е   │
│ SELECT pending → Send в stream → │
│ bosun-client применяет           │
└──────────────┬───────────────────┘
               │
               ▼
┌──────────────────────────────────┐
...
```
                    k8s sends SIGTERM (rolling update / HPA scale-down)
                       ...
                       └───────────────┬───────────────────┘
                                       ...
                                       ▼
                       ┌───────────────────────────────────┐
                       │ Все другие pods увидят флаг на    │
                       │ следующем 5s bosun_pods refresh:  │
                       │ active_pods исключает pod_2.      │
                       │ Новые Subscribe'ы НЕ роутятся на  │
                       │ pod_2 (≤5s окно — pod_2 сразу их  │
                       │ redirect'нет).                    │
                       └───────────────────────────────────┘
...
```
       CI/CD pipeline: build bundle → sign → push в chiit-server
              ...
              │ → INSERT bosun_commands_queue    │
              │   (per-host записи)              │
              │ → pod-polling-loop'ы каждые 500ms│
              │   подберут команды своих hosts   │
              └──────────────┬───────────────────┘
                              │
                              ▼
              ┌──────────────────────────────────┐
