...
date: 2026-05-19
status: draft (rev 3, pending review)
...
Текущий bundle в MVP — один `main.star`, один `defaults/main.yaml`, плоский `templates/*.j2`. Это работает для демо с одним nginx-ресурсом. Реальный парк имеет много ролей (`postgres-chiit` — 16 ролей, 94 Go-файла, 37 шаблонов, 25 иерархических inventory yaml). Текущий формат не масштабируется и не дает явного разделения по ролям.
...
├── main.star                     # entry point в корне; bundle.toml.entry опционален, default = "main.star"
...
- `main.star` в корне — единственный entry point. Внутри загружает inventory и роли через `load`. Поле `bundle.toml.entry` опционально; если опущено, используется `"main.star"`.
...
# entry        = "main.star"          # optional, default = "main.star"
...
Парсер: `serde::Deserialize` из toml в `struct BundleMetadata { name, version, description: Option<String>, requires_bosun, entry, tags: BTreeMap<String, String>, inventory: BundleInventoryConfig }`. Поле `entry` помечено `#[serde(default = "default_entry")]` с возвратом `"main.star"`; в `bundle.toml` его можно опустить. Все строки должны быть валидным UTF-8. `[bundle.tags]` — документация для CLI `--help`, во время evaluate не валидируется.
...
4. Запускает starlark eval `main.star`:
...
- Прямой вызов из `main.star` в корне bundle (top-level или функция, определённая в этом же файле) — **запрещён**. Корневой `main.star` — это orchestration, не config content. Author кладёт template в роль или lib, оттуда зовёт `template()`. Реакция: `fail("template: cannot be called from bundle entry (main.star); move rendering into a role or @lib module")`.
...
- Если `module = <root>/main.star` (entry в корне bundle) → `BundleError::TemplateFromEntry` (см. выше про запрет).
...
pub enum BundleError {
    ...
    TemplateFromEntry { hint: String },                               // NEW (бывший TemplateFromManifests)
...
requires_bosun = "^0.4"
# entry опущен — используется default "main.star"
...
`examples/nginx-demo/bundle/main.star`:
...
Что произойдёт:
1. `Bundle::load_dir(examples/nginx-demo/bundle)` читает `bundle.toml`, применяет default `entry = "main.star"`, валидирует через path_safety.
2. CLI дедуплицирует, сортирует, формирует `tags = {"production"}`, пишет Prometheus textfile, лог.
3. Evaluator стартует с `main.star`.
...
- `Bundle::resolve_template` для разных module-context'ов, включая reject из корневого `main.star`.
...
- Render из корневого main.star → reject TemplateFromEntry.
...
Feature: Bundle directory structure
  ...
  Scenario: template() from root main.star is rejected
    Given a bundle where main.star (в корне) calls template("foo.j2") at top-level
    When I apply the bundle
    Then exit code is 3
    And stderr contains "cannot be called from bundle entry"
...
- 3 — manifest/eval error (новые сценарии: ModuleNotFound, UnsupportedLoadPath, PrivateSymbol, cross-module template, tags require fail, TemplateFromEntry, DefaultMergeStrategyMissing, PathSafety попадают сюда)
- 4 — CLI/окружение
...
- Директория `manifests/` убрана из layout: entry-манифест (`main.star`) лежит непосредственно в корне bundle.
- `bundle.toml.entry` стало опциональным; default — `"main.star"`. Существующие bundle, которые явно указывали `entry = "manifests/main.star"`, должны быть переложены и поле `entry` либо удалено, либо изменено на `"main.star"`.
- `BundleError::TemplateFromManifests` переименован в `BundleError::TemplateFromEntry`; текст ошибки — «template() cannot be called from bundle entry (main.star)».
- Обновлены fixture-bundle (BDD, examples/nginx-demo, examples/multi-role-pg) и тестовые helpers под новый layout.
...
- `template()` переработан под call-stack walk (function-defining-module approach): резолвится по defining file текущего user-frame'а, с зафиксированным fallback на per-Module bound closure если 0.13 API не подходит. Reject `template()` из корневого `main.star`.
